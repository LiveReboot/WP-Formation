...
class wp_bootstrap_navlist_walker extends Walker_Nav_Menu
{
    ...
    public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
    {
        ...
        if (stristr($item->attr_title, 'disabled')) {
            $output .= $indent . '<li role="presentation" class="disabled nav-item"><a  class="nav-link" name="' . esc_attr($item->title) . '">' . esc_attr($item->title) . '</a>';
        } else {
            $output .= $item->current ? $indent . '<li class="active nav-item">' : $indent . '<li>';
        }
